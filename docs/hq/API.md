...
### `GET /v1/summary`
```json
{ "mode": "paper|live|mixed", "updatedAt": "…",
  ...
  "treasury": { "address": "…", "sol": "0", "cia": "0" } }
```
Trading P&L never includes creator fees; fees are their own line.
...
{ "id": 1, "number": "001", "name": "…", "cat": "sprite id", "skin": "skin id",
  ...
  "stats": { "balanceSol": "0", "portfolioSol": "0",
             ...
             "roiPct": "0" } }
```
`coin` may be null (an agent with no coin of its own). `roiPct` is realized + unrealized
trading P&L over net deposits, and is null when nothing is deposited.
...
{ "limits": { "maxPerTradeSol": "…", "maxOpenPositions": 0, "stopLossPct": "…", "takeProfitPct": "…",
  ...
  "positions": [ { "mint": "…", "symbol": "…", "costSol": "…", "valueSol": "…", "entryPrice": "…",
                   "price": "…", "pnlSol": "…", "pnlPct": "…", "openedAt": "…" } ],
  "decisions": [ Decision ], "trades": [ Trade ],
  "equity": [ { "t": "…", "portfolioSol": "…" } ],
  "fees": [ { "t": "…", "sol": "…", "tx": "…" } ],
  "transfers": [ { "t": "…", "kind": "deposit|withdrawal", "sol": "…", "tx": "…" } ],
...
Decision: `{ "kind": "decision", "id": "…", "t": "…", "agentId": 1, "action": "buy|sell|hold", "mint": "…|null", "symbol": "…|null", "reason": "…", "mode": "…" }`

Trade: `{ "kind": "trade", "id": "…", "t": "…", "agentId": 1, "side": "buy|sell", "mint": "…", "symbol": "…",
          "sol": "…", "tokens": "…", "price": "…", "pnlSol": "…|null", "pnlPct": "…|null",
          "trigger": "strategy|stop_loss|take_profit|trailing_stop|daily_limit|manual", "tx": "…|null", "mode": "…" }`
(`tx` is null only in paper mode.)
...
### `GET /v1/treasury`
`{ "address": "…", "sol": "…", "cia": "…", "flows": [ { "t": "…", "kind": "fee_in|profit_in|buyback|funding_out|funding_in", "sol": "…", "tx": "…" } ] }`

### `GET /v1/stream` (Server-Sent Events)
Events: `trade`, `decision`, `promotion`, `buyback`, `fee`, `summary` — each `data:` is the
same JSON object the endpoints above return for that kind. A client reconnects with
`Last-Event-ID`.

### `POST /v1/perks/verify`
Body: `{ "wallet": "…", "message": "…", "signature": "base58" }`. The message is the one
`GET /v1/perks/challenge?wallet=…` returned (a nonce with an expiry; single use). HQ checks
the signature and the wallet's $CIA balance on chain, and answers
...
early access to a pick or a trade (that would let holders trade ahead of the public).
