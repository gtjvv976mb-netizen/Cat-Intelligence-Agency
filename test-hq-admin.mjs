...
section("THE COMMANDS");
{
  ...
  ok("an unknown command: refused, naming the commands", await refused(() => run({ op: "agent.promote", id: 1 })) === "unknown_command" && COMMANDS.length === 12);
  /* intent.resolve: settled from the chain by the executor, never deleted here */
  ok("intent.resolve with no executor to read the chain: refused, the marker untouched", await (async () => { db.createIntent({ id: "stuck-a", agentId: 1, wallet: addr(101), kind: "buy" }); return (await refused(() => run({ op: "intent.resolve", id: "stuck-a" }))) === "no_executor" && db.getIntent("stuck-a").state === "prepared"; })());
  const asked = [];
  const resolveDeps = { ...deps, executor: { resolveIntent: async (id, opts) => { asked.push([id, opts]); return { resolved: true, state: "abandoned" }; } } };
  const res = await executeAdminCommand({ op: "intent.resolve", id: "stuck-a", acceptLanded: true }, { source: "test", deps: resolveDeps });
  ok("…with one, the executor settles it, told whether the owner accepts a landed one", res.resolved === true && res.state === "abandoned" && asked[0][0] === "stuck-a" && asked[0][1].acceptLanded === true && res.wallet === addr(101));
  ok("…a marker that does not exist: 404", await (async () => { try { await executeAdminCommand({ op: "intent.resolve", id: "nope" }, { source: "test", deps: resolveDeps }); return false; } catch (e) { return e.clause === "no_intent" && e.status === 404; } })());
...
section("THE CLI'S LINES, AND KEYGEN'S REFUSALS");
{
  ...
  ok("intent resolve, with --landed to close one the chain says landed", JSON.stringify(c("intent resolve abc-123")) === JSON.stringify({ op: "intent.resolve", id: "abc-123" }) && c("intent resolve abc-123 --landed").acceptLanded === true);
  ok("buyback abandon / retry", JSON.stringify(c("buyback abandon b-1")) === JSON.stringify({ op: "buyback.abandon", id: "b-1" }) && c("buyback retry b-1").op === "buyback.retry");
