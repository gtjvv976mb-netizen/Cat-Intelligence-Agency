...
- **Live execution** goes through the existing pre-sign checks (Jupiter's
  ...
  one per wallet across processes; the chain settles it on every indexer pass (its signature
  landed and read back, or failed, or — only once its blockhash was seen valid or two minutes
  have passed, the chain is past its last valid height, and a last status read still finds
  nothing — expired), so neither a restart nor an RPC outage ever trades twice or leaves a stop
  loss blocked. The owner can resolve a stuck marker from the chain (`intent resolve`).
- **The books are the chain's**: each live agent's ledger is rebuilt from its wallet's own
  ...
  are never trading P&L. Return is over the SOL ever deposited; the drawdown and the equity chart
  value open positions at their quotes, with deposits, withdrawals and fees neutral; a position
  with no quote for an hour is valued at the lower of its last price and its cost, and at 0 after
  a day; a token moved out leaves at its value, a loss realized but never a gain; and cash + rent +
  positions at cost always equals net deposits + fees + realized + other. Ranks follow
  career realized trading profit, never demote, and are only cosmetic.
...
| `test-hq-ledger.mjs` | the ledger from recorded mainnet wallets: a deposit, a round trip, empty claims and withdrawals; a creator-fee claim that is never trading P&L; each classification rule; the indexer's pages and cache; the runtime's books and the contract's stats; a paper ledger by hand; the accounting identity over every recorded wallet; a Jupiter sell that opens its wSOL account booked as the loss it is; tokens moved out at cost; return over what was deposited; drawdown and chart from marked values with flows neutral; a period's return without fees; a token moved out at its value (0 when stale), a loss realized and a gain never (no rank from a quote); a coin that stops quoting valued down by the contract's stale rule, with markAt and unpricedPositions; each fee sent once; an indexer that never skips a transaction and retries |
| `test-hq-risk.mjs` | every limit's fence; every buy refusal on its own and in order; exits at their exact edges; the daily loss limit tripping, holding and resetting; Crying Cat blocking recorded coins made to fail each line and anything unreadable; the agency's own coins never bought; a curve buy's ceiling held to the size the limits allowed; a coin that stops quoting shown by the stale rule while no stop or daily limit acts on it; through the runtime, a failing coin refused before any quote and the stops selling while paused and under the kill switch |
| `test-hq-execution.mjs` | paper signs nothing; live refused without both switches, the owner's RPC and the agent's own mode, and by the wallet itself; the in-flight marker before the build and the signature before the send; a restart that never trades twice (waiting, landed, failed, expired by its own blockhash, abandoned); a blockhash the node has not seen yet never read as expired (seen valid first or two minutes after signing, the block height guard, a last status read); an RPC outage that settles without a restart; a landed transaction waited for until it is read back, with protective transactions still going and the owner's resolve (buys refused until the indexer has read it, the indexer reading on around a transaction the RPC will not return); a Jupiter sell sized up to what the wallet holds; a simulation with no post-state refused; one open marker per wallet across two processes; HQ's own transactions checked before a signature and money only to the treasury; a pump.fun buy passing the bots' buy check and the simulation, and a draining one stopped |
...
| `test-hq-buyback.mjs` | $CIA as mainnet returned it (Token-2022, 6 decimals, HYPE-quoted curve); the two-leg path and its allowlist refusing a recorded Jupiter swap for another pair; the budget; revenue only from HQ's memos; off by default in every way; a run end to end with the SOL it cost read from the chain; the spend summed over a thousand and one buybacks; a leg signed but unconfirmed counted as spent and finished by the next run; one stopped between legs finished first and listed once; a leg or burn the chain recorded as failed never taken as landed; a second leg refused run after run stopped with its HYPE recorded, so later buybacks run; the owner's abandon and retry |
| `test-hq-perks.mjs` | the challenge, exactly the contract's six lines; good, bad, replayed, expired, mismatched and unknown signatures; keys of small order refused (forged signatures Node itself accepts); a wallet's newer challenge replacing its older one; the balance read from the chain, its failure in HQ's own words; holder exactly when the tier is not none; the tiers from config; perks only cosmetic or a say |
| `test-hq-admin.mjs` | signed admin requests (owner, stale, replayed, tampered, for another server, a small-order owner key, off without an owner); the owner's admin client; every command and its refusals (live needs the wallet typed back, back to paper only once nothing is held, skins follow the rank, $CIA never an agent's coin, intent.resolve only through the executor); the CLI's lines; keygen refusing where its output could be logged |
| `test-hq-api.mjs` | the whole server over HTTP on a scripted chain: every endpoint against the contract's JSON Schema, paper and live never added together, the rug check on every buy HQ made, the stream replayed from Last-Event-ID and live, capped per client network (/24, /48), ended after its maximum age, losing nothing across a burst, a slow reader or a resume from far back, a reset when the id is gone, and a reader that drains nothing cut, CORS only for the site and localhost, rate limits by the edge's client address, leaderboard periods never an object's own properties, request timeouts, /health in states only, open markers settled by the indexer, the number formats, and no secret, path, upstream or internal error in any answer |
...
| `test-hq-deploy.mjs` | the image carries HQ's whole import graph and its own dependencies; the Railway settings in the guide; HQ out of the extension build; the workflow pinned and secret-free; the owner's guide naming every variable HQ reads, with the code's defaults, and saying where a paper agent's deposit shows, the port Railway detects, to liquidate before going back to paper, and what to do with a stuck marker or buyback |
