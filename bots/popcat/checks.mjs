 ...
 *   creator_share      the creator's wallet holds at most MAX_CREATOR_SHARE_PCT of the supply
 ...
 *                      "not reached" (info) when the coin traded so much that its creation lies
 *                      beyond SAME_SLOT_LIMITS pages of signatures
 ...
 *   not_cashcat        neither its mint nor its creator is CashCat's
 */
...
export async function creationAndSameSlot({ rpc, curve, creator }) {
  ...
  for (let page = 0; page < SAME_SLOT_LIMITS.maxSignaturePages; page++) {
    ...
    all = all.concat(sigs);
    if (sigs.length < 1000) { oldest = sigs.length ? sigs[sigs.length - 1] : oldest; break; }
    before = sigs[sigs.length - 1].signature;
  }
  if (!oldest) return { createSig: null, createSlot: null, createTime: null, sameSlotBuyers: [], sameSlotTxs: null, pagesRead: all.length };
  ...
  return { createSig: oldest.signature, createSlot: oldest.slot, createTime: oldest.blockTime ?? null, sameSlotBuyers: [...buyers], sameSlotTxs: inSlot.length };
}
...
export function evaluate({ coin, onchain, creatorLaunches, metadata, now, cashcat = { mints: new Set(), wallets: new Set() } }) {
  ...
  add("not_cashcat", !cashcat.mints.has(coin.mint) && !cashcat.wallets.has(coin.creator), cashcat.mints.has(coin.mint) || cashcat.wallets.has(coin.creator) ? "a CashCat coin" : "creator and mint are not CashCat's");
  ...
  const creatorHeld = onchain.holders.filter((h) => h.owner === coin.creator).reduce((s, h) => s + h.amount, 0n);
  ...
  const n = onchain.sameSlotBuyers.length;
  ...
  else add("same_slot_buyers", n <= T.MAX_SAME_SLOT_BUYERS, n === 0 ? `none in slot ${onchain.createSlot}` : `${n} in slot ${onchain.createSlot}`);
